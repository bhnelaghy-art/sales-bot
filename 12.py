...
# 2. تعريف الدوال (يجب أن تكون قبل استخدامها)
...
if user_input := st.chat_input("أدخل البيانات..."):
    ...
    with st.chat_message("assistant"):
        ...
        if st.session_state.step == "ask_name":
            ...
            if len(user_input.split()) < 3: response = "عذراً، يرجى إدخال الاسم الثلاثي بالكامل."
            elif clean_name in existing_names: response = "هذا الاسم مسجل لدينا مسبقاً."
            else:
            ...
            if len(phone_clean) != 11 or not phone_clean.startswith(('010', '011', '012', '015')):
                ...
                response = "هذا الرقم مسجل مسبقاً."
                ...
                else: response = "تعذر التسجيل بسبب خطأ تقني."
                st.session_state.step = "ask_name"
